...
from extradite.worker import WIRE_SHM_BYTES_TAG
...
class ExtraditeSession:
    ...
    def _encode_for_child(
        ...
        use_shared_memory: bool = effective_policy == "value"
        ...
        if isinstance(value, set) is True:
            ...
            for item in value:
                encoded_item: object = self._encode_for_child(
                    item,
                    call_policy=call_policy,
                    shared_memory_names=shared_memory_names,
                )
        ...
        if isinstance(value, frozenset) is True:
            ...
            for item in value:
                encoded_item = self._encode_for_child(
                    item,
                    call_policy=call_policy,
                    shared_memory_names=shared_memory_names,
                )
        ...
        if isinstance(value, dict) is True:
            ...
            for key, item in value.items():
                encoded_key: object = self._encode_for_child(
                    key,
                    call_policy=call_policy,
                    shared_memory_names=shared_memory_names,
                )
    ...
    def construct_instance(
        ...
        for key, value in kwargs.items():
            encoded_kwargs[key] = self._encode_for_child(
                value,
                call_policy=call_policy,
                shared_memory_names=shared_memory_names,
            )
    ...
    def _encode_batch_call_specs_for_child(
        ...
        for call_index, call_spec in enumerate(call_specs):
            ...
            for key_obj, value in kwargs_obj.items():
                ...
                encoded_kwargs[key_obj] = self._encode_for_child(
                    value,
                    call_policy=call_policy,
                    shared_memory_names=shared_memory_names,
                )
    ...
    def set_attr(
        ...
        """
        shared_memory_names: list[str] = []
        encoded_value: object = self._encode_for_child(
            value,
            call_policy=call_policy,
            shared_memory_names=shared_memory_names,
        )
